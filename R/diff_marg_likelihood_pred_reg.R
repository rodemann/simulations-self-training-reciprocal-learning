...
diff_marg_likelihood_pred_reg <- function(labeled_data,
                                          unlabeled_data,
                                          test_data,
                                          target,
                                          glm_formula) {
  ...
  for (i in seq(n_imp)) {
    ...
    models_pseudo <- lapply(data_sets_pred, function(data){
      logistic_model <- glm(formula = formula,
                            data = data,
                            family = "binomial")
      list(logistic_model,data)
    })
    
    get_reg_crit = function(model){
      crit_int = NA
      try({
      regularizer = model[[2]] %>% unlist %>% abs %>% sum
      model = model[[1]] 
      crit_int = get_log_marg_l(model) + regularizer
      })
      if((is.na(crit_int) == TRUE)){
        regularizer
      }else{
        crit_int
      }
    ...
    crit <- lapply(models_pseudo, get_reg_crit)
    
    
    
    winner <- which.max(unlist(crit))
